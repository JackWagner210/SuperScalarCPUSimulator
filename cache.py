...
class Cache:
            ...
            for j in range(0, 2):
                ...
                if self.cacheSets[i][j][0] == 1 and self.cacheSets[i][j][1] == 1:
                    ...
                    if (wbAddr >= (self.numInstructions * 4) + 96):
                        self.dataval[
                            Setup.getIndexOfMemAddress(wbAddr, False, self.dataval, self.address, self.numInstructions)] = self.cacheSets[i][j][3]
                        # self.dataval[ self.getIndexOfMemAddress(wbAddr, False) ] = self.cacheSets[i][j][3]
                    if (wbAddr + 4 >= (self.numInstructions * 4) + 96):
                        self.dataval[
                            Setup.getIndexOfMemAddress(wbAddr + 4, False, self.dataval, self.address, self.numInstructions)] = self.cacheSets[i][j][4]
    ...
    def checkCache(self, dataIndex, instructionIndex, isWriteToMem, dataToWrite):
        ...
        if address1 < 96 + (4 * self.numInstructions):  # cant use dataIndex because aligning might have bridged boundry!
            data1 = self.instructions[
                Setup.getIndexOfMemAddress(address1, False, self.dataval, self.address, self.numInstructions)]
        ...
        if address2 < 96 + (4 * self.numInstructions):
            data2 = self.instructions[
                Setup.getIndexOfMemAddress(address2, False, self.dataval, self.address, self.numInstructions)]
        else:
        ...
        if address1 in self.justMissedList:
            ...
            # back if both blocks are on the data side.  Takes care of the case where you are across the boundary.
            ...
            if self.cacheSets[setNum][self.lruBit[setNum]][1] == 1:
                ...
                if (wbAddr + 4 >= (self.numInstructions * 4) + 96):
                    self.dataval[
                        Setup.getIndexOfMemAddress(wbAddr + 4, False, self.dataval, self.address, self.numInstructions)] = \
